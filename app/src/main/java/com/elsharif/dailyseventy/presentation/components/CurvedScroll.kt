...
@OptIn(ExperimentalComposeUiApi::class)
@Composable
fun CircularScrollView(
    itemCount: Int,
    modifier: Modifier = Modifier,
    fullCircle: Boolean = false, // 👈 لو true يبقى دايرة كاملة، لو false يبقى نص دايرة
    item: @Composable (Int) -> Unit
) {
    val size = remember { mutableStateOf(IntSize.Zero) }
    ...
    Box(
        ...
        Layout(
            ...
            val radius = (minOf(layoutWidth, layoutHeight) / 2.5f) // 👈 نص قطر الدائرة
            val centerX = layoutWidth / 2
            val centerY = layoutHeight / 2

            val angleStep =
                if (fullCircle) (2 * PI / itemCount) else (PI / (itemCount + 1))
